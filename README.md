...
**[Try the live demo →](https://vitreous-playground.vercel.app)** Ask it how to train for a gran fondo. Watch it find the contradiction, stop, and hand you the decision.

---

## What this is

Vitreous is a working hypothesis about how AI should be built into high-stakes software — and, eventually, a framework that makes that hypothesis cheap to apply.

The name comes from the vitreous body of the eye — the transparent medium you see through, not the thing you look at. The same instinct as a "glass box", without pretending to own the term.
...
A clarification on scope. Vitreous does not tell you *when* to reach for a model versus when to stay in deterministic code — that decision is upstream of the framework and belongs to whoever is designing the system. Vitreous's job begins the moment you have decided you need probabilistic reasoning. From there, it is a tool for making that reasoning legible and correctable, not for minimising it.
...
The core of Vitreous is not a component library. It is a way of modelling an AI-assisted run so that supervision is intrinsic to the data, not bolted on at the view layer.
...
None of these primitives is novel in isolation. The contribution Vitreous is trying to make is treating them as a *single coherent substrate* — a shared vocabulary of nodes, events, and redactions — so the same model works whether you are building clinical decision support, a financial advisor's workbench, a legal drafting tool, or a compliance review interface. The domain logic changes. The supervision substrate does not.

## Where this sits among its neighbours

The ideas above have neighbours, and it is worth being precise about where the overlap ends. Most of these tools are complements to Vitreous, not competitors — the difference is almost always *who the reasoning surface is for*.

**Agent observability platforms** — [Langfuse](https://langfuse.com), [Arize Phoenix](https://phoenix.arize.com), [Laminar](https://laminar.sh), and trace-rendering component libraries like [AgentPrism](https://evilmartians.com/chronicles/debug-ai-fast-agent-prism-open-source-library-visualize-agent-traces) — share Vitreous's instinct that everything should be recorded and replayable. But they are built for engineers, after the fact: dashboards you consult when something went wrong. Vitreous takes the same record-everything discipline and points it at the *end user, live, inside the product*. The trace is not a debugging artifact; it is the interface.

**Agent-frontend stacks and protocols** — [CopilotKit](https://github.com/copilotkit/copilotkit) and the [AG-UI protocol](https://docs.ag-ui.com) solve the transport problem: streaming agent state into a frontend, generative UI, and human-in-the-loop hooks you can wire up however you like. They are deliberately unopinionated about supervision semantics. Vitreous is the opinionated layer that transport leaves open: a fixed node vocabulary, confidence that must trace to citations, conflicts that halt rather than average, gates on every mutation. Nothing prevents a Vitreous surface from being fed over AG-UI.

**Backend interrupt mechanisms** — LangGraph's interrupts and tools like HumanLayer give you pause-and-approve as a backend primitive. They answer "how does the run stop?" but not "what does the user see, and what vocabulary do they reason in while deciding?" Vitreous is mostly about the second question.

**Event-sourced agent architectures** — recent research ([ESAA](https://arxiv.org/html/2602.23193), [event-sourced reactive agent graphs](https://arxiv.org/pdf/2605.21997)) and practitioner writing have converged on the append-only log as the right substrate for agent auditability. Vitreous agrees, and treats that finding as settled infrastructure: the interesting work is what you build *on top of* the log for the person who has to trust the output.

If you are choosing between Vitreous and any of these, you are probably not choosing — you are stacking.
...
It is less interesting if you are building open-ended consumer chat, creative tooling, or anywhere the cost of a hallucination is low and the cost of friction is high. Vitreous deliberately adds friction in exchange for legibility. That trade is right for some surfaces and wrong for others. Pick accordingly.

## Current state

- **Concept and principles** — this document, plus the original design note in [`docs/concept.md`](./docs/concept.md).
- **Core substrate** — `@vitreous/core` implements the typed DAG, the append-only event log with replay, deterministic ID generation, and the privacy/redaction layer described above. This is the part of the project the rest of the framework will be built around.
- **First renderer** — `@vitreous/react` is an early React layer over the core: spatial rail, decision provenance, execution gates, conflict resolver. It is one reference implementation, not the framework. Names and shapes at this layer will change.
- **First adapter** — `@vitreous/adapter-anthropic` puts the substrate under a live Anthropic (Claude) tool-use loop: retrievals become citation nodes, contradictions halt the loop for arbitration via an injected `flag_conflict` tool, state-changing tools suspend at approval gates until a human resolves them, and the final answer arrives through a `record_decision` tool whose provenance must reference the sources actually relied on. This is the bridge for anyone with a chat column: classify your tools as source/gated/plain, and supervision falls out of the loop.
- **Working playground** — `apps/playground` wires the full loop against a live model: a chat surface backed by a real LLM call, grounded in local source documents, emitting Vitreous events as it reasons. The seeded scenario is deliberately adversarial — two endurance-training studies that flatly contradict each other — so a single question ("how should I train for a gran fondo?") walks the whole pattern: citations added, conflict detected, stream halted, human arbitrates, decision made with provenance, execution gated. With a `GEMINI_API_KEY` set it runs against the live model; without one it falls back to a scripted replay of the same scenario — the supervision flow (arbitration, gating, forking) stays fully interactive either way, which is what a hosted demo runs.
...
Longer-form writing on the thinking behind Vitreous is forthcoming on Substack. A link will land here when it does.
